...
export function AuthForm({ mode }: { mode: 'login' | 'signup' }) {
  ...
  return <main className="relative grid min-h-screen overflow-hidden bg-ivory-100 lg:grid-cols-[1.05fr_.95fr]">
    ...
    <section className="relative hidden overflow-hidden bg-navy-950 p-12 lg:flex lg:flex-col lg:justify-between">
      <div className="relative z-10 flex items-center gap-3 text-white"><span className="grid h-10 w-10 place-items-center rounded-xl bg-white/10 font-heading text-lg font-black">N</span><span className="font-heading text-xl font-extrabold">Niyukti</span></div>
    ...
    <section className="relative z-10 flex items-center justify-center px-5 py-10 sm:px-8">
      <div className="w-full max-w-md rounded-3xl border border-stone-200/60 bg-white/80 p-6 shadow-premium backdrop-blur-md sm:p-9">
        <Link href="/" className="focus-ring inline-flex items-center gap-2 rounded-lg text-xs font-bold text-navy-800 lg:hidden"><span className="grid h-7 w-7 place-items-center rounded-lg bg-navy-950 text-white">N</span> Niyukti</Link>
